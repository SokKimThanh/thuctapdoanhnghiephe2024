<script type="text/javascript">
	...
	/**
	 * Đoạn mã này là một tập hợp các hàm JavaScript dùng để quản lý và xử lý 
	 * slug (đường dẫn thân thiện) cho các bài viết trên một trang web. Các hàm 
	 * này thực hiện các chức năng khác nhau liên quan đến việc tạo, xem trước, 
	 * và kiểm tra slug. Dưới đây là phân tích chi tiết về từng hàm:
	 */
	/* Slug */
	...
	/**1. slugPreview(title, lang, focus=false)
	 * Hàm này tạo slug từ tiêu đề và cập nhật các trường HTML tương ứng với giá trị slug mới.
	 * Tham số:
	 * title: Tiêu đề từ đó slug được tạo.
	 * lang: Ngôn ngữ của slug (ví dụ: "vi" hoặc "en").
	 * focus: Một tham số boolean chỉ ra liệu hàm có được gọi khi người dùng đang nhập liệu hay không.
	 * Hoạt động:
	 * Sử dụng hàm slugConvert để chuyển đổi tiêu đề thành slug.
	 * Cập nhật giá trị của các trường HTML với ID tương ứng với lang với slug mới.
	 */
	...
	/**2. slugPreviewTitleSeo(title, lang)
Hàm này cập nhật xem trước tiêu đề SEO dựa trên tiêu đề hiện tại.

Tham số:

title: Tiêu đề từ đó slug được tạo.
lang: Ngôn ngữ của tiêu đề.
Hoạt động:

Kiểm tra nếu trường tiêu đề SEO (#title${lang}) có giá trị.
Nếu không, sử dụng tiêu đề để cập nhật trường xem trước tiêu đề SEO.
 */
	...
	/**3. slugPress()
	Hàm này gắn sự kiện keyup cho các trường input liên quan đến slug để tự động cập
	 nhật slug khi người dùng nhập liệu.

	Hoạt động:
	Lấy danh sách ngôn ngữ (sluglang).
	Gắn sự kiện keyup cho các trường input có lớp for-seo trong card-article.
	Cập nhật slug và tiêu đề SEO khi người dùng nhập liệu.
	 */
	function slugPress() {
		var sluglang = "vi,en";
		var inputArticle = $('.card-article input.for-seo');
	...
	/**4. slugChange(obj)
	Hàm này thay đổi trạng thái của input slug dựa trên trạng thái của một checkbox.

	Tham số:

	obj: Đối tượng checkbox.
	Hoạt động:

	Nếu checkbox được chọn, đặt các trường input slug thành readonly.
	Nếu không, bỏ thuộc tính readonly của các trường input slug.
	 */
	...
	/**5. slugStatus(status)
	Hàm này cập nhật slug và tiêu đề SEO dựa trên trạng thái (status).

	Tham số:

	status: Trạng thái để xác định cách cập nhật slug (1 là theo tiêu đề mới, 0 là theo tiêu đề cũ).
	Hoạt động:

	Cập nhật slug và tiêu đề SEO dựa trên trạng thái.
	 */
	...
	/**6. slugAlert(result, lang)
	Hàm này hiển thị cảnh báo slug dựa trên kết quả kiểm tra slug.

	Tham số:

	result: Kết quả kiểm tra slug (1: thành công, 0: thất bại, 2: trạng thái trung gian).
	lang: Ngôn ngữ của slug.
	Hoạt động:

	Hiển thị hoặc ẩn cảnh báo tương ứng với kết quả kiểm tra slug.
	 */
	...
	/**7. slugCheck()
	Hàm này kiểm tra sự tồn tại của slug bằng cách gửi yêu cầu AJAX đến server.

	Hoạt động:
	Lấy danh sách ngôn ngữ (sluglang).
	Lặp qua các trường input slug và gửi yêu cầu AJAX để kiểm tra sự tồn tại của slug.
	Hiển thị cảnh báo dựa trên kết quả từ server.
	 */
	function slugCheck() {
		var sluglang = "vi,en";
